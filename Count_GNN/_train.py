...
from utils import anneal_fn, get_enc_len, load_data, get_linear_schedule_with_warmup, \
    GetEdgeAdj, split_and_batchify_graph_feats,bp_compute_large10_abmae,bp_compute_large20_abmae,bp_compute_abmae
from cnn import CNN
from rnn import RNN
from txl import TXL
...
from EATTS import EGATS
from ESUMS import ESUMS
...
def train(model, optimizer, scheduler, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    if config["reg_loss"] == "MAE":
        ...
        reg_crit = lambda pred, target: bp_compute_abmae(F.relu(pred), target)+F.l1_loss(F.relu(pred), target)+F.mse_loss(F.relu(pred), target)
    ...
    if config["bp_loss"] == "MAE":
        ...
        bp_crit = lambda pred, target, neg_slp: F.mse_loss(F.leaky_relu(pred, neg_slp), target)+F.l1_loss(F.leaky_relu(pred, neg_slp), target)
        ...
        bp_crit = lambda pred, target, neg_slp: bp_compute_abmae(F.leaky_relu(pred, neg_slp), target)+F.l1_loss(F.leaky_relu(pred, neg_slp), target, reduce="none")+F.mse_loss(F.leaky_relu(pred, neg_slp), target)
    elif config["bp_loss"] == "MSEABMAE":
        bp_crit = lambda pred, target, neg_slp: bp_compute_abmae(F.leaky_relu(pred, neg_slp), target)+F.mse_loss(F.leaky_relu(pred, neg_slp), target)
    else:
        raise NotImplementedError
    ...
    for batch_id, batch in enumerate(data_loader):
        ...
        s=time.time()
        pred,filmreg = model(pattern, pattern_len, pattern_e_len, graph, graph_len, graph_e_len,p_e_max,g_e_max,p_index,g_index)
...
def evaluate(model, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    with torch.no_grad():
        for batch_id, batch in enumerate(data_loader):
            ...
            pattern.to(device)
            graph.to(device)
            pattern_len, graph_len, counts,p_e_max, g_e_max, pattern_e_len, graph_e_len,p_index,g_index = pattern_len.to(device), graph_len.to(device), counts.to(device), p_e_max.to(device), g_e_max.to(device),\
...
if __name__ == "__main__":
    ...
    elif train_config["model"] == "EGATS":
        model = EGATS(train_config)
    ...
    if all([os.path.exists(os.path.join(train_config["save_data_dir"],
                                        "%s_%s_dataset.pt" % (
                                                data_type, "dgl" if train_config["model"] in ["RGCN", "RGIN", "PPN",
                                                                                              "EDGEMEAN","EDGESUM","EGATS","EDGESUMS"] else "edgeseq")))
